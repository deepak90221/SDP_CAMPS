  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (isLogin) {
        ...
        const response = await axios.post('/register', { name: username, email, password });
      ...
      setError(error.response ? error.response.data.error || 'An error occurred.' : 'Network error occurred.');
    }
  };
