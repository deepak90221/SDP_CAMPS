...
const CodingExam = () => {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden && !submitted && warningCount < 3) {
        alert('Warning: You are switching tabs. Make sure to submit your exam before leaving this page.');
        setWarningCount(warningCount + 1);
      } else if (document.hidden && !submitted && warningCount === 3) {
        submitExam();
      }
    };
  ...
  }, [timeLeft, submitted, timer, warningCount]); // Add warningCount as a dependency
